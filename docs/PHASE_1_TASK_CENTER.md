...
9. API 错误响应必须统一，避免直接暴露 Java 异常堆栈或不稳定的错误结构。
...
| 参数校验错误收敛 | `VALIDATION_FAILED` |
| JSON 解析错误收敛 | `INVALID_JSON` |
| 业务参数错误收敛 | `BAD_REQUEST` |
| 未知异常 traceId | `INTERNAL_SERVER_ERROR` |
...
```text
java-service/src/main/java/com/kailei/demo/model/ApiErrorResponse.java
java-service/src/main/java/com/kailei/demo/config/GlobalExceptionHandler.java
```
...
| `VALIDATION_FAILED` | 400 | `@Valid`、Bean Validation、方法参数校验失败 |
| `MISSING_PARAMETER` | 400 | 缺少必填 query 参数 |
| `INVALID_JSON` | 400 | 请求体 JSON 格式错误或字段类型不匹配 |
| `METHOD_NOT_ALLOWED` | 405 | 请求方法不支持 |
| `NOT_FOUND` | 404 | 接口不存在；实际生效依赖 Spring MVC 是否开启 no-handler 异常 |
| `INTERNAL_SERVER_ERROR` | 500 | 未预期异常 |

处理策略：

- 可预期的业务错误返回 4xx。
- 未预期错误返回 500，并记录 error 级别日志。
- 500 响应包含 `traceId`，便于从日志反查。
- 参数校验和 JSON 解析错误尽量把字段名放入 `details`。
...
- `README.md`：增加编辑接口、错误响应示例和状态流转说明。
...
- 已确认任务不允许编辑，并返回 `BAD_REQUEST`。
- 已调度任务不允许编辑，并返回 `BAD_REQUEST`。
- 已执行任务不允许整体取消，并返回 `BAD_REQUEST`。
- 非 FAILED 动作不允许重试，并返回 `BAD_REQUEST`。
- `operatorUserId` 与 `TaskPlan.userId` 不一致时，不允许写操作，并返回 `BAD_REQUEST`。
- 缺少 `operatorUserId` 时，如果任务已有 `userId`，不允许写操作，并返回 `BAD_REQUEST`。
- 请求体 JSON 不合法时，返回 `INVALID_JSON`。
- 请求参数校验失败时，返回 `VALIDATION_FAILED`。
...
4. 增加分页查询，避免任务列表全量返回。
5. 增加字段级编辑审计 diff。
6. 增加 task/action 级别的状态机服务，避免状态流转分散在业务代码中。
7. 将 `ApiErrorResponse` 错误码枚举化，避免后续硬编码字符串扩散。
