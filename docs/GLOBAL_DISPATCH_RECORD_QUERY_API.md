...
本次新增全局 dispatch records 查询接口，用于任务中心监控页展示失败、RUNNING、等待重试和重试耗尽列表。前端不需要先知道 `planId`，可以直接分页查询全局调度记录。
...
3. 如果同时传入 `status=RUNNING&retryExhausted=true`，由于条件矛盾，会返回空结果。
4. `startTime/endTime` 当前过滤的是 `triggerAt`，不是 `startedAt` 或 `finishedAt`。
...
2. 增加 `retryDue=true`，区分等待重试和已经到期可重试。
3. 增加按 `planId/actionId` 的可选过滤，形成统一查询接口。
4. 前端实现失败调度记录列表页和重试耗尽列表页。
